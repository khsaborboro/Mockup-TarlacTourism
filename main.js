...
/* =========================================================
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const filterDropdown = document.getElementById("activity-filter");
    const resetButton = document.getElementById("reset-filter");
    const dealCards = document.querySelectorAll(".deal-card"); // Selects all deals

    // 2. Create the filtering function
    function filterDeals() {
      // Get the current selected value and make it lowercase to avoid case-sensitive errors
      const selectedCategory = filterDropdown.value.toLowerCase();

      // Loop through every card
      dealCards.forEach(card => {
        // Get the category assigned to the card
        const cardCategory = card.getAttribute("data-category").toLowerCase();

        // If 'Select Activity' is chosen, show all cards
        if (selectedCategory === "select activity") {
          card.style.display = ""; // Clears the inline display none, putting it back in the CSS grid
        } 
        // Otherwise, check if the card matches the selected category
    ...
    filterDropdown.addEventListener("change", filterDeals);

    // 4. Listen for clicks on the reset button
    resetButton.addEventListener("click", () => {
      filterDropdown.value = "Select Activity"; // Reset dropdown text
      filterDeals(); // Run the filter function to show all cards again
    });
  });

  // SCRIPT TO HANDLE LOAD MORE for anao

 document.addEventListener("DOMContentLoaded", () => {
    const cards = document.querySelectorAll(".deal-card");
    const actionBtn = document.getElementById("load-more-btn");
    
    const initialCount = 3; // Number of cards to show initially
    let currentlyVisible = initialCount; 
    let isCollapsing = false; // Tracks if we are currently loading more or showing less

    // Function to hide/show cards based on the currentlyVisible counter
    function updateCardVisibility() {
      ...
      // Figure out if we need to switch directions and update button text
      if (currentlyVisible >= cards.length) {
        isCollapsing = true; // We hit the max, time to start going down
        actionBtn.textContent = "Show Less";
      } else if (currentlyVisible <= initialCount) {
        isCollapsing = false; // We hit the min, time to start going up
        actionBtn.textContent = "Load More";
      } else {
        // Keep the text based on the current direction we are heading
        actionBtn.textContent = isCollapsing ? "Show Less" : "Load More";
      }
    }
    ...
    // When the button is clicked, step up by 3 or step down by 3
    actionBtn.addEventListener("click", () => {
      if (isCollapsing) {
        currentlyVisible -= 3; // Subtract 3 cards
        
        // Safety check: ensure it doesn't accidentally drop below the initial 3
        if (currentlyVisible < initialCount) {
          currentlyVisible = initialCount; 
        }
      } else {
        currentlyVisible += 3; // Add 3 cards
      }
    ...
    //ANAO
   const items = document.querySelectorAll('.collage-img');
  const io = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting){
        const group = entry.target.closest('.col-collage').querySelectorAll('.collage-img');
        group.forEach((el, i) => {
          setTimeout(() => el.classList.add('pop-in'), i * 140);
        });
        io.unobserve(entry.target);
      }
    });
  ...
  document.querySelectorAll('.col-collage').forEach(c => {
    io.observe(c.querySelector('.collage-img'));
  });
